...
export default function MensFellowshipPage() {
  ...
  const isMale = (sex: unknown) => {
    const s = String(sex ?? "").trim().toLowerCase();
    return s === "male" || s.includes("male") || s === "m" || s.includes("man");
  };
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        const men = res.data.data.filter((m) => isMale((m as any).sex));
        setItems(men);
      } catch {
        if (cancelled) return;
    ...
    <div className="space-y-4">
      <div>
        ...
        <p className="text-muted-foreground">
          Men members list for fellowship.
        </p>
      </div>
